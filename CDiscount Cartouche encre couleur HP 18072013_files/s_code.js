...
function s_doPlugins(s) {
    ...
    if (cm_sp) {
        ...
        if (cm_sp.indexOf("RP_") == 0) { s.eVar69 = cm_sp; }
        else {
    ...
    if (cid) {
        ...
        if (cid == "1234") { s._channel = "Site Referant"; }
    }

    //Renomme les mots clés inconnus CM2.55
    if (!cid || cid == "search") { if (s._keywords == "Keyword Unavailable") { s._keywords = "Mot Cle Inconnu"; StackDirect = s._channel = refinc; } }
    ...
    s.prop16 = tpA[1]; // Set hour
    s.prop17 = tpA[2];  // Set day
    s.prop18 = tpA[3]; // Set Weekend / Weekday
    ...
    /* Recherche Interne */
    if (s.eVar9) {
        if (s.prop7 == "Recherche Interne:Category") { s.eVar4 = s.eVar3 = s.prop7; }
        if (s.prop7 == "Recherche Interne:Front") { s.eVar4 = s.eVar3 = s.prop7; }
        if (s.eVar65) { s.eVar4 = s.eVar3 = "Recherche Interne:Front:Connexe"; }
    }
    ...
    if (s.products) {
        var pr; pr = s.products; var a, i, tq, tr, pc = ""; if (pr) { a = pr } else { a = pr } var d, sku, q, r, e, f = new Array(); d = []; sku = []; q = []; r = []; e = []; f = []; var b, c = new Array(); if (a) { if (a.indexOf(",", 0) > -1) { b = a.split(","); } else { b = ["" + a + ""]; } pc = b.length; for (i = 0; i < pc; i++) { c = b[i].split(";"); d.push(c[0]); sku.push(c[1]); if (c[2] > 0) { q.push(c[2]) }; if (c[3] > 0) { r.push(c[3]) }; e.push(c[4]); f.push(c[5]); } tq = eval(q.join('+')); tr = eval(r.join('+')); }
    }
    ...
    //Annule commande extreme
    if (s.purchaseID) {
        if (tr > 50000) { s.disableTracking = true; }
    }
